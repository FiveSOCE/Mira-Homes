...
public final class MiraHomesPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getServer().getPluginManager().registerEvents(new HomeGuiListener(gui), this);
            ...
            command.setExecutor((sender, cmd, label, args) -> {
                if (!(sender instanceof org.bukkit.entity.Player player)) {
                    sender.sendMessage(LEGACY.deserialize(
                            CHAT_PREFIX + "&cMiraHomes can only be opened by a player."));
                    return true;
                }
                gui.open(player);
                return true;
        ...
        getLogger().info("MiraHomes v" + getPluginMeta().getVersion()
                + " enabled. Successful home teleports are visualized by MiraCosmetics when installed.");
    }
}
